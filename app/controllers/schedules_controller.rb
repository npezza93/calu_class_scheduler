class SchedulesController < ApplicationController
  ...
  def index
    @courses = @user.schedules
    @offerings = @courses.collect { |course| Offering.find(course.offering_id) }
  end
  ...
  def new
    ...
    @categories = CurriculumCategory.where(major: User.find(params[:user_id]).major_id).collect { |cat| cat.id }
    @user_courses = Transcript.where(user_id: params[:user_id]).collect { |c| c.course_id }
    ...
    Transcript.where(user_id: params[:user_id]).find_each do |c|
