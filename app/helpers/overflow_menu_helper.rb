...
module OverflowMenuHelper
  def overflow_menu(resource, edit_link: nil, destroy_link: nil, other: nil)
    content_tag(:div, class: "relative overflow-menu") do
      material_icon.more_vert.css_class("menu-toggle pointer").to_s +
        content_tag(:div, class: "mdc-simple-menu", tabindex: -1) do
          overflow_menu_items(resource, edit_link, destroy_link, other)
        end
    end
  end
  ...
  def overflow_edit_link(resource, link)
  ...
  def overflow_destroy_link(resource, link)
  ...
  def overflow_menu_items(resource, edit_link, destroy_link, other)
    content_tag(:div, class: "mdc-simple-menu__items mdc-list",
                      role: :menu, aria: { hidden: true }) do
      ((overflow_edit_link(resource, edit_link) if edit_link).to_s +
        (overflow_destroy_link(resource, destroy_link) if destroy_link).to_s +
